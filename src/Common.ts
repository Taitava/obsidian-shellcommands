...
export async function getFileContentWithoutYAML(file: TFile): Promise<string> {
    return new Promise((resolve) => {
        ...
        const file_content = this.app.vault.read(file);
        file_content.then((file_content: string) => {
            const frontmatter_cache: FrontMatterCache = this.app.metadataCache.getFileCache(file).frontmatter;
