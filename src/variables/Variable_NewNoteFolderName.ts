...
export class Variable_NewNoteFolderName extends Variable {
    ...
    protected generateValue(): string {
        ...
        return folder.isRoot() ? "." : folder.name; // If the folder is the vault's root folder, return "." instead of " " (a space character). I don't know why the name is " " when the folder is root.
    }
}